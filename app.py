...
.shap-bar-pos { background: #ef4444; height: 22px; border-radius: 4px; display: inline-block; }
.shap-bar-neg { background: #3b82f6; height: 22px; border-radius: 4px; display: inline-block; }
...
# Barre de navigation
with st.sidebar:
    st.markdown("## 🏦 ISM Scoring Credit")
    st.markdown("---")
...
# Fonction SHAP maison 
def compute_shap_logreg(model, input_df):
    """
    Calcule une approximation des valeurs SHAP pour une Régression Logistique
    via la méthode des coefficients pondérés.
    Fonctionne avec un Pipeline scikit-learn.
    """
    try:
        preprocessor = model[:-1]   # tout sauf le dernier estimateur
        classifier   = model[-1]    # LogisticRegression
        ...
        # Contribution = coef * valeur_transformée
        ...
        for col in original_cols:
            mask = [col.lower() in name.lower() for name in feature_names]
            shap_values[col] = float(np.sum(contributions[mask])) if any(mask) else 0.0
...
if page == "📊 Prédiction":
    ...
    with col_result:
        if predict_btn:
            ...
            shap_values = compute_shap_logreg(model, input_data)

            if shap_values:
                st.markdown('<div class="card" style="margin-top:1rem"><div class="card-title">🔍 Explication SHAP — Pourquoi cette décision ?</div>', unsafe_allow_html=True)
                st.caption("🔴 Rouge = augmente le risque · 🔵 Bleu = réduit le risque")
                ...
                for feature, value in sorted_shap:
                    bar_width = int(abs(value) / max_val * 150)
                    color     = "#ef4444" if value > 0 else "#3b82f6"
                    direction = "↑ Risque +" if value > 0 else "↓ Risque −"
                    col_a, col_b, col_c = st.columns([2, 2, 1])
                    col_a.markdown(f"**{feature}**")
                    col_b.markdown(f'<div style="background:{color};height:22px;width:{bar_width}px;border-radius:4px;margin-top:4px"></div>', unsafe_allow_html=True)
                    col_c.markdown(f'<span style="color:{color};font-size:0.8rem;font-weight:600">{direction}</span>', unsafe_allow_html=True)
...
#  PAGE 3 — À PROPOS
    ...
    with col1:
        st.markdown("""
        <div class="card">
            <div class="card-title">🤖 Modèle utilisé</div>
            ...
            <p><strong>Précision :</strong> ~99%</p>
    ...
    with col2:
        ...
        <div class="card">
            ...
            <p><strong>Déséquilibre :</strong> Corrigé par class_weight</p>
        </div>""", unsafe_allow_html=True)
    ...
    <div class="card">
        ...
        <table width="100%" style="border-collapse:collapse;font-size:0.9rem;text-align:center">
            <tr style="background:#F2EDE6">
                <th style="padding:10px">Métrique</th>
                <th style="padding:10px">Valeur</th>
                <th style="padding:10px">Interprétation</th>
            </tr>
            <tr>
                <td style="padding:10px;text-align:left"><strong>AUC-ROC</strong></td>
                <td style="padding:10px;color:#1A7A3C;font-weight:700">0.9999</td>
                <td style="padding:10px">Quasi-parfait — discrimine très bien défaut/non-défaut</td>
            </tr>
            <tr style="background:#FAF6F0">
                <td style="padding:10px;text-align:left"><strong>Précision</strong></td>
                <td style="padding:10px;color:#1A7A3C;font-weight:700">~99%</td>
                <td style="padding:10px">99% des prédictions sont correctes</td>
            </tr>
            <tr>
                <td style="padding:10px;text-align:left"><strong>Rappel</strong></td>
                <td style="padding:10px;color:#1A7A3C;font-weight:700">Élevé</td>
                <td style="padding:10px">Détecte la grande majorité des clients à risque</td>
            </tr>
            <tr style="background:#FAF6F0">
                <td style="padding:10px;text-align:left"><strong>F1-Score</strong></td>
                <td style="padding:10px;color:#1A7A3C;font-weight:700">Élevé</td>
                <td style="padding:10px">Bon équilibre précision/rappel</td>
            </tr>
        </table>
    </div>
    """, unsafe_allow_html=True)
    ...
    <div class="card">
        ...
        <p>
            <strong>SHAP (SHapley Additive exPlanations)</strong> est une méthode d'interprétabilité 
            des modèles de Machine Learning basée sur la théorie des jeux.
        </p>
        <p>
            Pour chaque prédiction, SHAP calcule la <strong>contribution de chaque variable</strong> 
            à la décision finale :
        </p>
        <ul>
            <li>🔴 <strong>Contribution positive (rouge)</strong> → la variable <em>augmente</em> le risque de défaut</li>
            <li>🔵 <strong>Contribution négative (bleue)</strong> → la variable <em>réduit</em> le risque de défaut</li>
        </ul>
        <p>
            Pour une Régression Logistique, les valeurs SHAP sont calculées exactement 
            via les <strong>coefficients pondérés</strong> : <code>SHAP(xᵢ) = coefᵢ × xᵢ</code>
        </p>
        <p style="background:#F2EDE6;padding:0.8rem;border-radius:8px;margin-top:0.8rem">
             Les explications SHAP apparaissent automatiquement après chaque prédiction 
            sur la page <strong>Prédiction</strong>.
        </p>
    </div>
    """, unsafe_allow_html=True)
    ...
    <div class="card">
        ...
        <table width="100%" style="border-collapse:collapse;font-size:0.9rem">
            <tr style="background:#F2EDE6">
                ...
                <th style="padding:8px;text-align:center">Importance estimée</th>
            </tr>
            <tr><td style="padding:8px">1</td><td>RATIO_ENDETTEMENT</td><td style="text-align:center"><code>Numérique</code></td><td style="text-align:center">⭐⭐⭐⭐⭐</td></tr>
            <tr style="background:#FAF6F0"><td style="padding:8px">2</td><td>SECTEUR_ACTIVITE</td><td style="text-align:center"><code>Catégorielle</code></td><td style="text-align:center">⭐⭐⭐⭐⭐</td></tr>
            <tr><td style="padding:8px">3</td><td>GENRE</td><td style="text-align:center"><code>Catégorielle</code></td><td style="text-align:center">⭐⭐⭐⭐</td></tr>
            <tr style="background:#FAF6F0"><td style="padding:8px">4</td><td>REGION</td><td style="text-align:center"><code>Catégorielle</code></td><td style="text-align:center">⭐⭐⭐⭐</td></tr>
            <tr><td style="padding:8px">5</td><td>TAILLE_ENTREPRISE</td><td style="text-align:center"><code>Catégorielle</code></td><td style="text-align:center">⭐⭐⭐</td></tr>
            <tr style="background:#FAF6F0"><td style="padding:8px">6</td><td>TYPE_CARTE</td><td style="text-align:center"><code>Catégorielle</code></td><td style="text-align:center">⭐⭐⭐</td></tr>
            <tr><td style="padding:8px">7</td><td>GARANTIE</td><td style="text-align:center"><code>Catégorielle</code></td><td style="text-align:center">⭐⭐⭐</td></tr>
            <tr style="background:#FAF6F0"><td style="padding:8px">8</td><td>SITUATION_MATRIMONIALE</td><td style="text-align:center"><code>Catégorielle</code></td><td style="text-align:center">⭐⭐</td></tr>
            <tr><td style="padding:8px">9</td><td>TYPE_LOGEMENT</td><td style="text-align:center"><code>Catégorielle</code></td><td style="text-align:center">⭐⭐</td></tr>
            <tr style="background:#FAF6F0"><td style="padding:8px">10</td><td>TYPE_COMPTE</td><td style="text-align:center"><code>Catégorielle</code></td><td style="text-align:center">⭐</td></tr>
        </table>
    </div>
    """, unsafe_allow_html=True)
    ...
    <div class="card">
        ...
        <table width="100%" style="border-collapse:collapse;font-size:0.9rem">
            <tr style="background:#F2EDE6"><th style="padding:8px;text-align:left">Score</th><th style="padding:8px;text-align:center">Risque</th><th style="padding:8px;text-align:center">Décision</th></tr>
    ...
    <div class="card">
        ...
        <p><strong>Technologies :</strong> Python · Scikit-Learn · Streamlit · GitHub · Streamlit Cloud</p>
    </div>
    """, unsafe_allow_html=True)